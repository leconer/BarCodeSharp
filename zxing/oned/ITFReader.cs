...
namespace com.google.zxing.oned
{
	...
	public sealed class ITFReader:OneDReader
	{
		...
		public override Result decodeRow(int rowNumber, BitArray row, System.Collections.Hashtable hints)
		{
            ...
            if (startRange.IsBlank() || endRange.IsBlank()) return null;
		...
		{
			int? endStartNullable = skipWhiteSpace(row);
            if (endStartNullable.IsBlank()) return null;

            int endStart = (int)endStartNullable;
			int[] startPattern = findGuardPattern(row, endStart, START_PATTERN);
            if (startPattern.IsBlank()) return null;
			...
			var success = validateQuietZone(row, startPattern[0]);
		...
		internal int[] decodeEnd(BitArray row)
		{
			...
			try
			{
                ...
                if (endPattern.IsBlank()) return null;

				// The start & end patterns must be pre/post fixed by a quiet zone. This
				// zone must be at least 10 times the width of a narrow line.
				// ref: http://www.barcode-1.net/i25code.html
				var success = validateQuietZone(row, endPattern[0]);
                if (!success) return null;
