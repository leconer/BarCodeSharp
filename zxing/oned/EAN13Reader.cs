...
namespace com.google.zxing.oned
{
	...
	public sealed class EAN13Reader:UPCEANReader
	{
		...
		protected internal override int? decodeMiddle(BitArray row, int[] startRange, System.Text.StringBuilder resultString)
		{
            ...
            if (counters.IsBlank()) return null;
			...
			for (int x = 0; x < 6 && rowOffset < end; x++)
			{
                ...
                if (bestMatch.IsBlank()) return null;
			...
			bool success = determineFirstDigit(resultString, lgPatternFound);
            if (!success) return null;
			
			int[] middleRange = findGuardPattern(row, rowOffset, true, MIDDLE_PATTERN);
            if (middleRange.IsBlank()) return null;
		...
		private static bool determineFirstDigit(System.Text.StringBuilder resultString, int lgPatternFound)
		{
			for (int d = 0; d < 10; d++)
			{
